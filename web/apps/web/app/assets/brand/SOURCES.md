...
raster files cannot, so they are listed here. So are the two app icons in `web/apps/web/public/icons/`: they are
the icons of `public/site.webmanifest.json`, which needs fixed addresses, and everything in `public/` is served
as it is, so their notice is kept here.
...
| `public/icons/icon-192x192.png`, `public/icons/icon-512x512.png`                 | `mark.svg`, rendered at that size on a transparent background                          |

The code reads these files by name only. A new logo replaces the SVGs and renders the raster files (the app
icons included) again at the same sizes; no code changes.
