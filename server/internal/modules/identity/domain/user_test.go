...
func TestCheckUserPatchReportsEveryField(t *testing.T) {
	...
	tests := []struct {
		...
		{"NUL in the first name", UserPatch{FirstName: ptr("\x00")},
			[]shared.FieldError{{Field: "first_name", Code: "invalid_format", Message: "must not contain a NUL character"}}},
		{"NUL in the display name", UserPatch{DisplayName: ptr("a\x00")},
			[]shared.FieldError{{Field: "display_name", Code: "invalid_format", Message: "must not contain a NUL character"}}},
		{"one problem per field", UserPatch{FirstName: ptr(long + " x.io")},
			[]shared.FieldError{{Field: "first_name", Code: "too_long", Message: "must be at most 255 characters"}}},
