...
| 带 Nerve 版权声明的文件 | 0 | 18（新写的 1 个组件、3 个 SVG、2 个 `SOURCES.md`，改正的 12 个）；修复轮把 `public/icons/SOURCES.md` 并入 `app/assets/brand/SOURCES.md` 之后为 17 |
...
| M | 事项 |
|---|---|
| M4 | - 收集箱列表和工作项详情不再为 Plane 的收集箱机器人（`intake@plane.so`、名字带 `-intake`）显示特殊的头像和名字，创建者按普通用户显示。由系统代为创建的工作项怎样显示，随工作项和收集箱的接口一起定。<br>- 工作项动态里自动归档的操作者显示为站点名 `SITE_NAME`（基线写死 "Plane"）；系统操作者的显示同上。 |
| M8 | - **propel 的对应源码**：`@makeplane/propel` 以编译后的形式随 Nerve 分发（AGPL-3.0-only），它的上游仓库不公开。公开发布之前，取得提交 `0a31b15` 的源码，或从包里的 source map 还原（1376 个源文件中缺 `src/internal/variant-props.ts`），以便提供对应源码（2.9）。<br>- **版本号**：12 个包和 web 应用的 `version` 仍是 Plane 的 1.4.2，帮助菜单显示它；Nerve 的版本号随首次发布定，届时一起改；`version-number.tsx` 导入整个 `package.json`，客户端包因此带着全部依赖和版本，改为只取 `version`（整分支评审 M7）。<br>- **分享图的地址**：`og:image`、`twitter:image` 是相对地址（基线如此），有了公开地址之后写成绝对地址。 |
...
- 29 张封面照片的来源和许可（看起来是图库照片，Plane 随代码分发）：收尾查明来源；查不到许可的替换或删除。导览截图里的真人头像和人名一并查（整分支评审 M8）。
- 其余基线就有的事项见第 5 节；执行中发现的（`window.open` 没有 `noopener`、133 张没有引用的图片、导入分组的错标、守卫里从未存在的样本路径等）见 [P5 评审记录](../reviews/P5-brand-review.md)第 7 节。
