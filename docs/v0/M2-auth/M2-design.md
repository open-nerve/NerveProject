...
|---|---|---|---|---|
| `anonymous` | 客户端 IP | 600 / 100 | `httpserver`，每个公开操作 | 匿名请求 30/minute（`DEFAULT_THROTTLE_RATES`，`settings/common.py:140-144`）。Nerve 的页面每次加载都会不带令牌调用实例配置、时区和续期，同一出口 IP 后的团队很快就会碰到 30。它也是续期、退出按 IP 的外层上限 |
...
| `refresh_session` | 刷新令牌里的会话 id（解析令牌得到，不查库） | 20 / 10 | `identity` 适配器 | 无。正常的页面每 15 分钟续期一次，加上重试 |
...
| 续期、退出 | `anonymous` | `refresh_session`。令牌解析不出会话 id 时直接 401，不查库 | 0 |
| 实例配置、时区 | `anonymous` | — | 0 |
...
- **为什么这样分**（评审 I3、Codex I-3、控制者复核 N3）：
  ...
  - 续期、退出按会话 id 计数，按 IP 的外层上限是 `anonymous`：一个团队在同一出口 IP 后开很多标签页，不会互相挤占续期的额度。会话 id 就在令牌里，解析就能拿到，不需要查库。代价：知道某个会话 id 的人，可以用伪造的刷新令牌把这个会话的续期桶用完，让它暂时续不了期（会话不会被撤销，前端退避后重试，7.1）。会话 id 只出现在访问令牌和刷新令牌里，日志中只记它的摘要（8.4）。
  - 无效的令牌由认证之前的闸门限制（3.6）。
...
- **位置**：
  ...
  - 其余五个桶在 `identity` 的 HTTP 适配器里，在调用用例之前：登录的键里有邮箱，要先解码请求体；续期、退出的键是令牌里的会话 id；`password_user` 的键是当前账户。适配器通过自己声明的小接口使用 `platform/ratelimit`，超出时返回 `shared` 的限流错误，走"一条路"映射（3.11）。
...
| 限流 | 认证接口合计每 IP 10/min，匿名 30/min，API Key 60/min；`/api/v1` 的响应带 `X-RateLimit-Remaining`、`X-RateLimit-Reset`（`plane/apps/api/plane/api/views/base.py:120-126`） | 3.10 的八个桶；超出时 429 带 `Retry-After`，不加 `X-RateLimit-*` | P2 |
...
- **限流的 429**：所有操作都可能返回（顶层的 `x-problem-codes`）。登录、注册、续期、退出、修改密码另有自己的桶，见 3.10。
...
| `RateLimiter`（`AllowAll`） | `identity/adapter/http` | `platform/ratelimit` | 适配器测试：每个操作的键和桶；被后面的桶拒绝时前面的桶不扣 |
...
请求 → 请求 ID → 异常恢复 → 访问日志 → 安全响应头          （固定链，httpserver.NewServer）
     ...
     → 生成的代码：解码 JSON 请求体
     → handler：RequireActor；操作级限流（登录、注册、续期、退出、修改密码）；把生成的类型转成用例的入参
...
ratelimit:                     # 每个桶都有速率和突发；test 全部调高（3.10）
  ...
  refresh_session: {per_minute: 20,   burst: 10}
...
- **永远不记**：
  ...
  - 会话 id 的原文。理由（按第三稿重新推导，Codex I-2）：会话 id 不再能用来撤销会话（撤销要交出真实的旧令牌，3.5），但知道它的人可以用伪造的刷新令牌把这个会话的续期桶用完，让它暂时续不了期（3.10）。看日志的人不该有这个能力。日志里记 `session_ref`，即会话 id 的 SHA-256 的前 8 个字节（十六进制），足够把同一个会话的几条日志串起来；排查时由 `session_ref` 在数据库中找到会话，需要能读库的权限；
  - `Authorization` 请求头；
...
- **INFO**：
  ...
  - 密码哈希名额等待超时：当时排队的数量。
- **WARN**：
  - 发现刷新令牌被重复使用：`user_id`、`session_ref`、IP；
...
| 适配器 | 6.3 表中各端口的测试；`authn` 把 `Actor` 放进 `context` 并返回限流键，令牌无效时返回 401 的错误，数据库出错时返回别的错误；HTTP 适配器：操作级限流的键和桶，续期和退出的 `context` 带 4 秒的期限 |
...
- **交付物**：
  ...
  2. 限流：`platform/ratelimit`（突发、`AllowAll`）；`httpserver` 的限流中间件和认证之前的失败闸门；`identity` 适配器中登录、注册、续期和退出的桶；`server.trusted_proxies` 和不可信 `X-Forwarded-For` 的提醒（3.6、3.10）。
  3. 固定链上的安全响应头（8.3）。
...
- **完成线**：
  ...
  - 3.10 表中的每个桶都有测试；失败闸门用计数的假认证器证明超额后不再调用认证器。
...
| M14 | 3.9 的比较位置；日志中的会话 id | 3.9；8.4（第三稿重新推导了理由） |
...
| I-2 | 只凭旧代数就撤销，没有证明旧令牌是真的 | 3.4；3.5 的判定表；4.1、4.5 新表 `auth_refresh_tokens`，`auth_sessions.token_hash` 删除；8.4 重新推导 `session_ref` 的理由；9.1；A1、A4、A5 | P1（表）、P2（逻辑和测试） |
...
| N3 | 匿名桶 | 3.10 突发、`refresh_session`、提高 `anonymous`、`X-Forwarded-For` 的提醒；6.5；7.1、7.4"会话暂不可用"；9.4；9.6。最后一条（401 优先于 429）由 I-3 取代 | P2、P4 |
| N4 | 同 I-7 | 3.14 | P1 |
