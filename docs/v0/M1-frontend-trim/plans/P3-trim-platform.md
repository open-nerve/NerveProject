...
> 执行前先读本节之后的"控制者评审补充"：它修改了原型的用法、基点和数字的口径，与本节冲突时以它为准。
...
控制者 2026-09-23 复核了 spec 和本计划：计划可以执行。下面的裁定和修正与正文冲突时，以本节为准。

### spec 第 3 节的裁定

- 第 1–8、10–15 条照写的执行。每一条都是上级设计留给本 Phase 的局部决定，理由已写在 spec 里。
  - 第 2 条（注册失败显示在登录页）要做两件事：交接 M2（spec 7.1 已列），7.5 的临时核对脚本断言这一行为。
  - 第 3 条（删 `EIssueServiceType`，`/work-items/` 写成字面量）：每一处构造的实参都是常量，删掉参数就不是空壳；
    `endpoints.py` 核对过地址。
- 第 9 条：同意，不加 Task。基线就没人用的 355 个包导出交给收尾。
  - M1 设计 7.2 说的"knip 清零"，指 knip 报告的内容清零。
  - 一个包的导出是不是 API，要逐个判断，这和 570 个死键、142 张死图片是同一类工作。
  - P3 仍然要用每个 Task 的 `symref.mjs orphaned` 删掉本 Task 造成的包级孤儿。

### 执行方式的修正

1. **原型提交是对照答案，不能整体照搬。** 正文说"每个文件改成什么样，以原型提交为准"，这句改为下面几条。
   - **按 Task 的步骤一步步做**：删文件 → 类型检查引导 → 孤儿核对 → 文案 → 守卫 → lint 上限 → 门禁。
     每一步都跑步骤里写的命令，把实测输出和预期并排写进报告。
   - **可以参考原型，也可以运行原型脚本。**
     - 某个文件怎么改，可以用 `git show <原型提交> -- <路径>` 看原型的结果。
     - `$P3TMP/t<N>/` 的 Python 脚本可以运行。它们是精确替换，原文不在就失败。
     - 条件是：跑完一个脚本，或照原型改完一个文件，都要读它的 diff，确认每一处改动都对应本 Task 的一条说明
       或一个类型错误。
   - **禁止以提交、目录或文件列表为单位取原型的内容**：`git checkout <原型提交> -- <目录或多个文件>`、`git cherry-pick`、
     `git read-tree`、`git diff … | git apply` 都不许用。步骤里的核对命令（类型检查、knip、`symref`、`deadvocab`、`lintdiff`）
     本身就是删除的向导。跳过它们，就等于没有人独立做过一遍。
   - **与原型的差异可以有，但每一处都要写明原因。**
     - 用 `git diff --cached --stat <原型提交>` 比较；P2 的实测就好过原型。
     - 差异为空时，报告也要写：本 Task 自己核对了哪些风险点（见第 5 条），核对的结果是什么。
     - 原型里你认为错的地方，照你认为对的做，并写进报告。
2. **基点是本分支上一个 Task 的实际提交**，不是计划里写的原型哈希。
   - 适用的命令：`symref.mjs orphaned`、`deadvocab.mjs`、`keyref.mjs orphaned`、`assets-orphaned.py`，以及步骤中其他写着 `<基点>` 的命令。
   - Task 1 的基点是 `6d9692b`（它之后的两个文档提交不改代码）；控制者派发时给出每个 Task 的基点。
   - 计划里写的 `0d85d9d`、`381a34c` 等，只在两棵树相同时与实际基点等价。
   - `headers.sh 6d9692b` 和 `keyref.mjs orphaned 6d9692b` 这类对**整个 Phase 基线**的核对，仍然用 `6d9692b`。
3. **数字以实测为准**（P2 评审裁定 2）：lint 上限、shortstat、knip 计数、构建体积、键数都按实测写。
   - lint 上限：
     - 实测**低于**计划时，说明多删了什么，且必须是本 Task 的孤儿或退化结构。
     - 实测**高于**计划时，先找原因，不许把上限往上写。
   - knip 计数：多出来的条目才是问题，少了只需说明。
   - `check:types` 是 23 个任务，`make lint-web` 52 个，`make test-web` 15 个，`make build-web` 11 个。
4. **测试输出要干净**（P2 评审的裁定）：
   - Task 1–6 允许的 stderr 只有一种：编辑器测试里 linkify 的 "already initialized" 几行，它们是继承来的，Task 7 修掉。
   - 其他任何新的 stderr、警告或报错都算失败。
   - Task 7 之后，`make test-web` 不应有任何 stderr。
   - 可以写环境声明（例如 vitest 的 setup 文件），不可以写日志过滤。
5. **每个 Task 报告必答的风险点**：
   - **恒为假的化简**：逐个核对选中的分支与基点在 CE 中的实际取值一致。`isEpic`、`isAdmin`、`isPro`、service 类型等，
     在报告里列出"删掉的参数 → 选中的分支"。
   - **共享状态**（P2 的 `UniqueID` 回归）：本 Task 做了下面两类改动时，报告写明它是否只读、会不会跨实例共享：
     - 新增了模块级常量或可变对象；
     - 改了编辑器扩展的 `create`、`configure`、`addOptions`、`onCreate`。
     编辑器的交互测试必须通过。
   - **扩展点确实是空的**（Task 6）：每删一个扩展点，写出它在基点的 CE 实现是"空 / 常量 / 只转发"中的哪一种，
     并给出所在的文件和行。
   - **路由模块的导出**（Task 7 的 `loop.sh`、`unexport.mjs`）：`app/` 下路由文件的约定导出一个都不能动：
     `default`、`meta`、`links`、`loader`、`clientLoader`、`action`、`clientAction`、`ErrorBoundary`、`HydrateFallback`、`handle`。
     报告列出 `loop.sh` 每一轮去掉 `export` 或删掉的符号，按包分组。
6. **报告放在最后一条回复里**：子 agent 不能写报告文件，由控制者存档。内容包括：
   - 提交哈希；
   - 每一步的命令、实测输出和预期；
   - 与原型的差异及原因；
   - 第 5 条的风险点；
   - 没做到的事和疑问。
   状态写 `DONE`、`DONE_WITH_CONCERNS` 或 `BLOCKED`。
7. **一个 Bash 调用只跑一个 git 命令**，不把 `cd` 和 git 串在一起。计划里写成一行的两个 git 命令，分两次调用。
   - 多步逻辑写成 `$P3TMP` 下的脚本。
