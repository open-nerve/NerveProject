...
func TestRedactURL(t *testing.T) {
	tests := []struct {
		...
		{"postgresql scheme", "postgresql://nerve:secret@localhost/nerve", "postgresql://nerve:xxxxx@localhost/nerve"},
		{"any query key containing password", "postgres://localhost/nerve?sslpassword=secret", "postgres://localhost/nerve?sslpassword=xxxxx"},
		{"scheme without // is masked entirely", "postgres:nerve:secret@localhost/nerve", "xxxxx"},
		{"other scheme is masked entirely", "mysql://u:secret@h/db", "xxxxx"},
	}
...
func TestDatabaseConfigLogValueMasksPassword(t *testing.T) {
	var buf bytes.Buffer
	slog.New(slog.NewTextHandler(&buf, nil)).Info("x", "db", validConfig().Database)

	out := buf.String()
	if strings.Contains(out, "secret") {
		t.Errorf("log output leaks the password: %s", out)
	}
	for _, want := range []string{"db.url=postgres://nerve:xxxxx@localhost:5432/nerve", "db.max_conns=10", "db.auto_migrate=false"} {
		if !strings.Contains(out, want) {
			t.Errorf("log output lacks %q: %s", want, out)
		}
	}
